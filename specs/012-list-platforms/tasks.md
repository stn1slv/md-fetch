...
- [X] T001 Run `make test` and confirm all existing unit tests pass (green baseline per dev guidelines)
...
- [X] T002 Add `supported_platforms() -> list[tuple[str, bool]]` to `src/mdfetch/router.py`: return `sorted((domain, cls.MATCH_SUBDOMAINS) for domain, cls in _REGISTRY.items())`. Fully type-hinted, read-only, no side effects. Leave existing `supported_domains()` unchanged.
- [X] T003 [P] Add a unit test in `tests/unit/test_router.py` asserting `supported_platforms()` returns sorted `(domain, bool)` tuples, covers every domain in `supported_domains()` (this registry-derived equality is what guarantees SC-003: a newly registered provider appears automatically with no edit to the list code), and marks `medium.com`/`substack.com` as subdomain-matching while `dev.to` is exact-match.
...
- [X] T006 [US2] In `src/mdfetch/cli.py`, enhance the list-rendering loop so each entry from `supported_platforms()` renders `f"{domain} (and *.{domain})"` when its `matches_subdomains` flag is `True`, and the bare `domain` otherwise (FR-006).
- [X] T007 [US2] Add a unit test in `tests/unit/test_cli.py` asserting `--list-platforms` output contains `*.medium.com` (subdomain-annotated) and that an exact-match domain such as `dev.to` appears without a `*.` wildcard.
...
- [X] T008 Bump `version` in `pyproject.toml` from `0.7.1` to `0.8.0` (minor — user-facing feature; gates PyPI release + Homebrew auto-update).
- [X] T009 [P] Update `README.md`: document the `--list-platforms` flag with a usage example (per CLAUDE.md README-sync rule).
- [X] T010 Run `make lint`, `make typecheck`, and `make test`; confirm all pass with zero errors before completion.
