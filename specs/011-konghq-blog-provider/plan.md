...
clean_html(soup):                              # KongExtractor implementation
  ...
  4. strip chrome blocks inside content (decompose):
        ...
        "[class*='TableOfContents']"  (table-of-contents, when present)
...
Unlike the WordPress-based providers, Kong's per-component class names are Next.js CSS-module hashes (e.g., `Article_toc__LOyCI`, `Section_section__Grz_Y`) that change between site builds. The plan deliberately depends **only** on stable, human-authored companion classes (`type-article`, `rich-text-block`, `component`, `video`, `more-on-this`, `section-header-block`, `intro`, `order-top`) plus one substring match (`TableOfContents`) and one date regex. No selector relies on a hashed `__xxxxx` suffix. This is the single biggest maintenance risk and will be recorded as a `CLAUDE.md` gotcha during implementation.
